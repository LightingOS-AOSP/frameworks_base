...
/**
 * Helper functions for uploading to Lightning
 */
public final class LightningUtils {
    private static final String TAG = "LightningUtils";
    ...
    /**
     * Uploads {@code content} to Lightning
     *
     * @param content the content to upload to Lightning
                ...
                try {
                    ...
                    try {
                        ...
                        if (!key.isEmpty()) {
                            ...
                            String msg = "Failed to upload to Lightning: No key retrieved";
                            callback.onFail(msg, new LightningException(msg));
                        }
                    } finally {
                        urlConnection.disconnect();
                    }
                } catch (Exception e) {
                    callback.onFail("Failed to upload to Lightning", e);
                }
            }
        });
    }
    ...
    private static Handler getHandler() {
        if (handler == null) {
            HandlerThread handlerThread = new HandlerThread("LightningThread");
